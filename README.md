...
> **~54 500 строк Rust · 74 модуля · 31 инструмент агента · 1 333 зелёных теста ·
> 363 скилла · 122 169 карточек ML-концептов · 6 ГБ библиотеки статей ·
...
Требуется ключ DeepSeek (`DEEPSEEK_API_KEY` в окружении). Конфиг —
...
slash-completion над вводом (голый `/` — весь список из 21 команды с адаптивной
...
В TUI: `Enter` — отправить, `Tab` — автодополнение команд, `/help` — 21 команда,
`/` — весь список команд, `/new` — новая сессия, `/skill-search` — умный поиск
скиллов, `/theme` — темы, `Esc` — прервать/выйти, колесо мыши — прокрутка,
...
│ markdown-лог · панель «фон» · /bg · /skill-search · уведомления           │
└─────────────────────────────────────▼─────────────────────────────────────┘
┌───────────────────────────────────────────────────────────────────────────┐
│ АГЕНТНЫЙ ЦИКЛ           agent/mod.rs · execute.rs · compact.rs            │
│ промпт → LLM → tool calls → результат → LLM → … → finish                  │
│ детекторы doom/spiral · компактификация L1→L2→L3 (80%) · трасса           │
└───────────────────────────────────────────────────────────────────────────┘
├───────────────────────────────────────────────────────────────────────────┤
│ ГЕЙТ РАЗРЕШЕНИЙ: permissions.rs + execpolicy.rs · sandbox (Landlock)      │
└─────────┬──────────────────────┬──────────────────────┬───────────────────┘
┌────────────────────┐   ┌───────────────────────┐   ┌──────────────────────┐
│ 28 ИНСТРУМЕНТОВ    │   │ СУБАГЕНТЫ (task)      │   │ BgRegistry +         │
│ tools.rs           │   │ explore · plan ·      │   │ watcher              │
│ файлы · bash · web │   │ code_review ·         │   │ background.rs        │
│ память · скиллы ·  │   │ test_runner           │   │ снимок → панель и    │
│ патчи · MCP · ACP  │   │ РОЙ до 8 (swarm)      │   │ уведомления в TUI    │
└────────────────────┘   └───────────────────────┘   └──────────────────────┘
┌───────────────────────────────────────────────────────────────────────────┐
│ СЛОЙ ЗНАНИЙ: library.rs · digests.rs · ml_concepts.rs · skills.rs         │
│ 6 ГБ статей · дайджесты · 122K карточек · 363 скилла (BM25 + эмбеддинги)  │
└─────────┬──────────────────────┬──────────────────────┬───────────────────┘
┌────────────────────┐   ┌───────────────────────┐   ┌──────────────────────┐
│ DeepSeek V4 API    │   │ АРИАДНА (local)       │   │ ПИРЫ — 5 CLI         │
│ облако, reasoning  │   │ Qwen3.5-4B GRPO       │   │ claude·kimi·         │
│ интерливинг-       │   │ llama.cpp, GPU        │   │ codewhale·hermes·    │
│ мышление, 1M ctx   │   │ RU, fallback EN       │   │ openclaw             │
└────────────────────┘   └───────────────────────┘   └──────────────────────┘
```
