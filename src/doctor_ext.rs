...
pub fn builtin_checks() -> Vec<Check> {
    vec![
        Check {
            id: CheckId::DiskSpace,
            name: "диск (свободное место)",
        ...
        Check {
            id: CheckId::ShellAvail,
            name: "shell (bash/sh)",
            run: check_shell_avail,
        },
        Check {
            id: CheckId::PythonAvail,
            name: "python3",
            run: check_python_avail,
        },
        Check {
            id: CheckId::RegexEngine,
            name: "regex-движок",
            run: check_regex_engine,
        },
        Check {
            id: CheckId::SessionDir,
            name: "каталог сессий",
            run: check_session_dir,
        },
        Check {
            id: CheckId::TranscriptQuota,
            name: "квота транскриптов",
            run: check_transcript_quota,
        },
        Check {
            id: CheckId::ClockSkew,
            name: "сдвиг часов",
            run: check_clock_skew,
        },
        Check {
            id: CheckId::EnvProxy,
            name: "прокси (env)",
        ...
        Check {
            id: CheckId::LocaleUtf8,
            name: "локаль UTF-8",
            run: check_locale_utf8,
        },
    ]
}
...
fn check_disk_space(ctx: &CheckCtx) -> Outcome {
    ...
    match df {
        Ok(out) if out.status.success() => match parse_df_pk(&String::from_utf8_lossy(&out.stdout))
        {
            ...
            None => Outcome::warn(
                format!("не разобрать вывод `df -Pk {mount}`"),
                "проверьте вывод df вручную",
            ),
        },
        _ => Outcome::warn(
            "утилита df недоступна — свободное место не проверено",
            "установите coreutils (df)",
        ),
    }
}
...
fn find_mount_point<'m>(path: &Path, mounts: &'m [String]) -> Option<&'m str> {
    mounts
        .iter()
        .filter(|mp| path.starts_with(Path::new(mp.as_str())))
...
fn disk_outcome(avail_bytes: u64, capacity_pct: u16) -> Outcome {
    ...
    if avail_bytes < DISK_FAIL_BYTES {
        Outcome::fail(
            format!("свободно {avail} (занято {capacity_pct}%) — критически мало"),
            "освободите место: сессии и транскрипты требуют диск",
        )
    } else if avail_bytes < DISK_WARN_BYTES {
        Outcome::warn(
            format!("свободно {avail} (занято {capacity_pct}%) — на грани"),
            "освободите место до следующего длинного прогона",
        )
    } else {
...
fn check_git_repo(ctx: &CheckCtx) -> Outcome {
    ...
    match inside {
        ...
        _ => Outcome::warn(
            "не git-репозиторий — история изменений недоступна",
            "git init",
        ),
    }
}
...
fn check_shell_avail(_ctx: &CheckCtx) -> Outcome {
    ...
    match (bash, sh) {
        ...
        (None, Some(s)) => Outcome::warn(
            format!("bash не найден; fallback на sh: {}{extra}", s.display()),
            "установите bash — shell-инструмент рассчитан на него",
        ),
        (None, None) => Outcome::fail(
            format!("ни bash, ни sh в PATH{extra}"),
            "установите bash — shell-инструмент не сможет работать",
        ),
    }
}
...
fn check_python_avail(_ctx: &CheckCtx) -> Outcome {
    let path_var = std::env::var("PATH").unwrap_or_default();
    match (
        find_in_path("python3", &path_var),
        find_in_path("python", &path_var),
    ) {
        (Some(p), _) => Outcome::ok(format!("python3: {}", p.display())),
        (None, Some(p)) => Outcome::warn(
            format!("python3 не найден; есть только python: {}", p.display()),
            "установите python3 — скрипты и скиллы пишутся под него",
        ),
        (None, None) => Outcome::warn(
            "python3 не найден в PATH — Python-скрипты недоступны",
            "apt install python3",
        ),
    }
}
...
fn run_regex_probes(probes: &[RegexProbe]) -> Result<usize, String> {
    for p in probes {
        let re =
            regex::Regex::new(p.pattern).map_err(|e| format!("шаблон «{}»: {e}", p.pattern))?;
        let caps = re
            .captures(p.sample)
            .ok_or_else(|| format!("шаблон «{}» не совпал с образцом «{}»", p.pattern, p.sample))?;
        let got = caps
            .name(p.group)
            .map(|m| m.as_str())
            .ok_or_else(|| format!("в captures нет группы «{}»", p.group))?;
        if got != p.expect {
            return Err(format!(
                "группа «{}»: ожидали «{}», получили «{got}»",
                p.group, p.expect
            ));
        }
    }
...
fn check_regex_engine(_ctx: &CheckCtx) -> Outcome {
    match run_regex_probes(REGEX_PROBES) {
        Ok(n) => Outcome::ok(format!(
            "{n} сложных паттерна: компиляция и named captures ok"
        )),
        Err(e) => Outcome::fail(
            format!("regex-движок сломан: {e}"),
            "regex критичен для правил permissions — проверьте тулчейн/сборку",
        ),
    }
}
...
fn check_session_dir(ctx: &CheckCtx) -> Outcome {
    if !ctx.workspace.is_dir() {
        return Outcome::fail(
            format!("workspace не каталог: {}", ctx.workspace.display()),
            "укажите существующий каталог проекта",
        );
    }
    let dir = sessions_dir(ctx);
    if let Err(e) = std::fs::create_dir_all(&dir) {
        return Outcome::fail(
            format!("не создать {}: {e}", dir.display()),
            "проверьте права на .theseus",
        );
    }
    ...
    match std::fs::write(&probe, b"ok").and_then(|_| std::fs::remove_file(&probe)) {
        ...
        Err(e) => Outcome::fail(
            format!("запись в {}: {e}", dir.display()),
            "проверьте права/квоту каталога сессий",
        ),
    }
}
...
fn quota_outcome(total: u64, limit: u64) -> Outcome {
    ...
    if total > limit {
        Outcome::warn(
            format!("транскрипты занимают {total_h} при квоте {limit_h}"),
            "запустите `theseus session prune` — старые транскрипты будут удалены",
        )
    } else {
...
fn check_clock_skew(ctx: &CheckCtx) -> Outcome {
    let probe = ctx
        .workspace
        .join(format!(".clock-probe-{}", std::process::id()));
    ...
    let result = match created {
        Ok(meta) => match meta.modified() {
            ...
            Err(e) => Outcome::warn(
                format!("mtime probe-файла недоступен: {e}"),
                "проверьте поддержку mtime файловой системой",
            ),
        },
        Err(e) => Outcome::warn(
            format!("не создать probe-файл в workspace: {e}"),
            "проверка сдвига часов пропущена",
        ),
    };
...
fn assess_skew(delta_secs: i64) -> Outcome {
    ...
    if abs <= CLOCK_SKEW_WARN_SECS {
        Outcome::ok(format!(
            "сдвиг часов относительно FS: {delta_secs} с (в норме)"
        ))
    } else if delta_secs > 0 {
        Outcome::warn(
            format!("mtime свежего файла в будущем на {abs} с — системные часы отстают"),
            "синхронизируйте время: timedatectl set-ntp true",
        )
    } else {
        Outcome::warn(
            format!("mtime свежего файла в прошлом на {abs} с — системные часы спешат"),
            "синхронизируйте время: timedatectl set-ntp true",
        )
    }
}
...
fn proxy_outcome(http: Option<&str>, https: Option<&str>) -> Outcome {
    ...
    if invalid.is_empty() {
        ...
        Outcome::warn(
            format!("прокси без схемы: {}", invalid.join(", ")),
            "задавайте прокси полностью, например http://proxy.local:3128",
        )
    }
}
...
fn check_locale_utf8(_ctx: &CheckCtx) -> Outcome {
    ...
    let current = LOCALE_VARS
        .iter()
        .filter_map(|k| std::env::var(k).ok().map(|v| format!("{k}={v}")))
        .collect::<Vec<_>>()
        .join(" ");
    let shown = if current.is_empty() {
        "LC_ALL/LC_CTYPE/LANG не заданы".to_string()
    } else {
        current
    };
    Outcome::warn(
        format!("локаль без UTF-8 ({shown}) — юникод/иконки могут отображаться бито"),
        "export LANG=C.UTF-8",
    )
}
...
mod tests {
    ...
    impl TmpDir {
        fn new(tag: &str) -> Self {
            let dir =
                std::env::temp_dir().join(format!("theseus-doc-ext-{}-{tag}", std::process::id()));
    ...
    fn proc_mounts_parse_and_longest_prefix() {
        ...
        assert_eq!(
            find_mount_point(Path::new("/home/user/proj"), &mounts),
            Some("/home")
        );
    ...
    fn df_pk_standard_and_edge_cases() {
        ...
        assert!(
            parse_df_pk("Filesystem 1024-blocks Used Available Capacity Mounted on\n").is_none()
        );
    ...
    fn disk_space_smoke_on_real_fs() {
        ...
        assert!(
            out.detail.contains("свободно") || out.detail.contains("df"),
            "detail: {}",
            out.detail
        );
    }
    ...
    fn find_in_path_requires_executable() {
        ...
        assert_eq!(
            find_in_path("fakesh", &path_var).as_deref(),
            Some(exe.as_path())
        );
    ...
    fn transcript_quota_nested_files_and_boundary() {
        ...
        assert_eq!(
            quota_outcome(TRANSCRIPT_WARN_BYTES, TRANSCRIPT_WARN_BYTES).status,
            Status::Ok
        );
    ...
    fn proxy_outcome_pure_and_env_serialized() {
        ...
        assert_eq!(
            proxy_outcome(Some("http://proxy.local:3128"), None).status,
            Status::Ok
        );
        assert_eq!(
            proxy_outcome(Some("http://ok:1"), Some("socks5://s:2")).status,
            Status::Ok
        );
        ...
        assert_eq!(out.status, Status::Ok);
        assert!(
            out.detail.contains("http://proxy.local:3128"),
            "detail: {}",
            out.detail
        );
    }
    ...
    fn report_render_exact_format() {
        let entries = vec![
            ReportEntry {
                id: CheckId::GitRepo,
                name: "git-репозиторий",
                outcome: Outcome::ok("всё хорошо"),
            },
            ReportEntry {
                id: CheckId::DiskSpace,
                name: "диск",
                outcome: Outcome::fail("мало места", "очистите диск"),
            },
        ];
        let report = Report {
            entries,
    ...
    fn run_all_counts_and_consistency() {
        ...
        let actual_fail = report
            .entries
            .iter()
            .filter(|e| e.outcome.status == Status::Fail)
            .count();
