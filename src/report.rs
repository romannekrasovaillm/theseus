        ...
        match ev.kind.as_str() {
            ...
            "Accounting" => {
                ...
                stats.completion_tokens = stats
                    .completion_tokens
                    .max(get_u64(&ev.payload, "completion_t"));
            }
...
pub fn render_markdown(stats: &SessionStats, meta: &ReportMeta) -> String {
    ...
    match &stats.finish_summary {
        ...
        None => {
            let _ = writeln!(
                out,
                "Событие `Finished` отсутствует — сессия, похоже, оборвалась."
            );
        }
    }
    out
...
mod tests {
    ...
    fn parse_transcript_reads_theseus_format() {
        let text = [
            ...
            theseus_line(
                101,
                "ToolCall { name: \"Bash\", args: \"cargo test\", decision: \"allow\" }",
            ),
            theseus_line(
                103,
                "ToolResult { name: \"Bash\", preview: \"ok\", ok: true }",
            ),
        ]
        .join("\n");
    ...
    fn parse_transcript_reads_generic_kind_payload_format() {
        ...
        let events =
            parse_transcript(&json!({"kind": "Finished", "payload": "готово"}).to_string());
    ...
    fn parse_debug_tuple_variant_with_escapes() {
        let line = theseus_line(
            9,
            r#"Finished("сделано: \"пункт а\" и \\ и перевод\nстроки")"#,
        );
        let events = parse_transcript(&line);
        assert_eq!(events[0].kind, "Finished");
        assert_eq!(
            events[0].payload["value"],
            json!("сделано: \"пункт а\" и \\ и перевод\nстроки")
        );
    ...
    fn parse_debug_event_rejects_malformed() {
        ...
        assert!(
            parse_transcript(&theseus_line(1, "Compact { from_msgs: 3 to_msgs: 1 }")).is_empty()
        );
    ...
    fn compute_stats_counts_counters_and_maxima() {
        let events = vec![
            event(
                "Status",
                json!({"turns": 3, "est_tokens": 900, "mode": "normal", "ts": 100}),
            ),
            event(
                "Status",
                json!({"turns": 7, "est_tokens": 1800, "mode": "normal", "ts": 200}),
            ),
            event(
                "Accounting",
                json!({"calls": 4, "prompt_t": 1000, "completion_t": 200, "ts": 150}),
            ),
            event(
                "Accounting",
                json!({"calls": 9, "prompt_t": 2500, "completion_t": 700, "ts": 250}),
            ),
            event(
                "Compact",
                json!({"from_msgs": 40, "to_msgs": 12, "ts": 260}),
            ),
    ...
    fn compute_stats_finish_summary_and_duration_edges() {
        ...
        assert_eq!(
            (stats.finish_summary.as_deref(), stats.duration_secs),
            (None, 0)
        );
    }
    ...
    fn compute_stats_pairs_calls_with_results_fifo() {
        let events = vec![
            event(
                "ToolCall",
                json!({"name": "Bash", "args": "a", "decision": "allow", "ts": 100}),
            ),
            event(
                "ToolCall",
                json!({"name": "Read", "args": "f", "decision": "allow", "ts": 101}),
            ),
            event(
                "ToolCall",
                json!({"name": "Bash", "args": "b", "decision": "allow", "ts": 102}),
            ),
            // результаты: первый Bash — 30 с, второй — 38 с (FIFO по имени)
            event(
                "ToolResult",
                json!({"name": "Bash", "preview": "ok", "ok": true, "ts": 130}),
            ),
            event(
                "ToolResult",
                json!({"name": "Bash", "preview": "fail", "ok": false, "ts": 140}),
            ),
            event(
                "ToolResult",
                json!({"name": "Read", "preview": "ok", "ok": true, "ts": 105}),
            ),
            // результат без вызова — сирота, в длительности не идёт
            event(
                "ToolResult",
                json!({"name": "Grep", "preview": "сирота", "ok": true, "ts": 150}),
            ),
        ];
        let stats = compute_stats(&events);
        ...
        let durations: Vec<(&str, u64, bool)> = stats
            .slowest
            .iter()
            .map(|t| (t.name.as_str(), t.duration_secs, t.ok))
            .collect();
        assert_eq!(
            durations,
            [("Bash", 38, false), ("Bash", 30, true), ("Read", 4, true)]
        );
        // метка результата раньше метки вызова — насыщение до нуля, не паника
        let events = vec![
            event(
                "ToolCall",
                json!({"name": "Bash", "args": "a", "decision": "allow", "ts": 200}),
            ),
            event(
                "ToolResult",
                json!({"name": "Bash", "preview": "ok", "ok": true, "ts": 150}),
            ),
        ];
    ...
    fn render_markdown_limits_slowest_to_five() {
        ...
        assert!(
            !section.contains("tool6"),
            "лишняя строка в топ-5:\n{section}"
        );
    }
    ...
    fn end_to_end_realistic_transcript() {
        let text = [
            ...
            theseus_line(
                1_784_387_375,
                "Status { turns: 1, est_tokens: 1200, mode: \"normal\" }",
            ),
            theseus_line(
                1_784_387_380,
                "ToolCall { name: \"Read\", args: \"src/lib.rs\", decision: \"allow\" }",
            ),
            theseus_line(
                1_784_387_382,
                "ToolResult { name: \"Read\", preview: \"pub fn ...\", ok: true }",
            ),
            theseus_line(
                1_784_387_390,
                "ToolCall { name: \"Bash\", args: \"cargo test\", decision: \"allow\" }",
            ...
            theseus_line(
                1_784_387_430,
                "ToolResult { name: \"Bash\", preview: \"test result: ok\", ok: true }",
            ),
            theseus_line(
                1_784_387_431,
                "Accounting { calls: 2, prompt_t: 5200, completion_t: 900 }",
            ),
            theseus_line(
                1_784_387_432,
                "Status { turns: 2, est_tokens: 6100, mode: \"normal\" }",
            ),
            theseus_line(1_784_387_433, "Compact { from_msgs: 18, to_msgs: 7 }"),
            theseus_line(
                1_784_387_435,
                "Finished(\"тест починен: поправлена граница диапазона\")",
            ),
        ]
        .join("\n");
        let parsed = parse_transcript_full(&text);
        ...
        assert_eq!(
            stats.finish_summary.as_deref(),
            Some("тест починен: поправлена граница диапазона")
        );
