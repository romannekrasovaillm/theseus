        ...
        match self {
            FixAction::CreateConfig => {
                format!(
                    ...
                    global_config_path(home).display()
                )
            }
            FixAction::FixWorkspacePerms => {
            ...
            FixAction::CreateSessionDir => {
                format!(
                    "создать каталог сессий {}",
                    sessions_dir(workspace).display()
                )
            }
            FixAction::CreateSkillsDir => {
                format!(
                    "создать каталог скиллов {}",
                    skills_dir(workspace).display()
                )
            }
...
const ISSUE_KEYWORDS: &[(FixAction, &[&str])] = &[
    (
        FixAction::CreateConfig,
        &["конфиг", "config.toml", "config"],
    ),
    (
        FixAction::FixWorkspacePerms,
        &["прав", "запис", "permission", "denied"],
    ),
    (FixAction::CreateSessionDir, &["сесси", "session"]),
    (FixAction::CreateSkillsDir, &["скилл", "skill"]),
    (
        FixAction::TrimTranscripts,
        &["транскрипт", "квот", "transcript"],
    ),
];
...
fn issue_matches(action: FixAction, issue_lower: &str) -> bool {
    ISSUE_KEYWORDS
        .iter()
        .any(|(a, keywords)| *a == action && keywords.iter().any(|kw| issue_lower.contains(kw)))
}
...
fn apply_create_config(home: &Path) -> FixResult {
    ...
    let Some(parent) = path.parent() else {
        return FixResult::failed(
            FixAction::CreateConfig,
            format!("нет родителя у {}", path.display()),
        );
    };
    if let Err(e) = std::fs::create_dir_all(parent) {
        return FixResult::failed(
            FixAction::CreateConfig,
            format!("не создать {}: {e}", parent.display()),
        );
    }
        ...
        .open(&path)
    {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
            return FixResult::skipped(
                FixAction::CreateConfig,
                format!("конфиг уже существует: {}", path.display()),
            );
        }
        Err(e) => {
            return FixResult::failed(
                FixAction::CreateConfig,
                format!("не создать {}: {e}", path.display()),
            )
        }
    };
    ...
    match file.write_all(EXAMPLE_CONFIG.as_bytes()) {
        Ok(()) => FixResult::done(
            FixAction::CreateConfig,
            format!("создан пример конфига: {}", path.display()),
        ),
        Err(e) => FixResult::failed(
            FixAction::CreateConfig,
            format!("запись в {}: {e}", path.display()),
        ),
    }
}
...
fn apply_workspace_perms(workspace: &Path) -> FixResult {
    ...
    let meta = match std::fs::metadata(workspace) {
        ...
        Err(e) => {
            return FixResult::failed(
                FixAction::FixWorkspacePerms,
                format!("metadata {}: {e}", workspace.display()),
            );
        }
    };
    if !meta.is_dir() {
        return FixResult::failed(
            FixAction::FixWorkspacePerms,
            format!("не каталог: {}", workspace.display()),
        );
    }
    ...
    if mode == 0o755 {
        return FixResult::skipped(
            FixAction::FixWorkspacePerms,
            format!("права уже 0755: {}", workspace.display()),
        );
    }
    match std::fs::set_permissions(workspace, std::fs::Permissions::from_mode(0o755)) {
        Ok(()) => FixResult::done(
            FixAction::FixWorkspacePerms,
            format!("права {}: {mode:04o} → 0755", workspace.display()),
        ),
        Err(e) => FixResult::failed(
            FixAction::FixWorkspacePerms,
            format!("chmod {}: {e}", workspace.display()),
        ),
    }
}
...
fn apply_workspace_perms(workspace: &Path) -> FixResult {
    FixResult::skipped(
        FixAction::FixWorkspacePerms,
        format!(
            "chmod поддерживается только на unix: {}",
            workspace.display()
        ),
    )
}
...
mod tests {
    ...
    impl TmpDir {
        fn new(tag: &str) -> Self {
            let dir =
                std::env::temp_dir().join(format!("theseus-doc-fix-{}-{tag}", std::process::id()));
    ...
    fn plan_maps_all_issue_kinds_in_canonical_order() {
        ...
        write_sized(
            &ws.join(".theseus/events-1.jsonl"),
            (TRANSCRIPT_QUOTA_BYTES + 1) as usize,
        );

        let plan = plan_fixes(&home, &ws, &all_issues());
        assert_eq!(
            plan,
            FixAction::ALL.to_vec(),
            "все 5 действий в каноническом порядке, без дублей"
        );
    }
    ...
    fn plan_ignores_unknown_and_empty_issues() {
        ...
        fs::create_dir_all(&ws).unwrap();
        assert!(
            plan_fixes(&home, &ws, &[]).is_empty(),
            "пустой вход — пустой план"
        );
        let unknown = vec![
        ...
        assert!(
            plan_fixes(&home, &ws, &unknown).is_empty(),
            "неизвестные проблемы не мапятся"
        );
    }
    ...
    fn plan_skips_already_satisfied_targets() {
        ...
        assert!(
            plan_fixes(&home, &ws, &all_issues()).is_empty(),
            "всё удовлетворено — план пуст"
        );

        // уберём каталог скиллов — в плане должно остаться ровно одно действие
        fs::remove_dir_all(skills_dir(&ws)).unwrap();
        assert_eq!(
            plan_fixes(&home, &ws, &all_issues()),
            vec![FixAction::CreateSkillsDir]
        );
    }
    ...
    fn apply_create_config_idempotent_and_never_overwrites() {
        ...
        assert_eq!(
            fs::read_to_string(global_config_path(&home)).unwrap(),
            original
        );
    ...
    fn apply_trim_transcripts_real_quota_end_to_end() {
        ...
        assert!(res.did_something, "detail: {}", res.detail);
        assert!(
            res.detail.contains("удалено 1 events-файлов"),
            "detail: {}",
            res.detail
        );
        assert!(
            !ws.join(".theseus/events-1000.jsonl").exists(),
            "старый удалён"
        );
        assert!(
            ws.join(".theseus/events-2000.jsonl").exists(),
            "новый остался"
        );
        ...
        assert!(!second.did_something);
        assert!(
            second.detail.contains("квота не превышена"),
            "detail: {}",
            second.detail
        );
    }
    ...
    fn dry_run_renders_plan_and_empty_case() {
        ...
        let ws = tmp.path().join("ws");
        let text = dry_run(
            &[FixAction::CreateConfig, FixAction::CreateSkillsDir],
            &home,
            &ws,
        );
    ...
    fn summary_renders_exact_format() {
        let results = vec![
            FixResult::done(
                FixAction::CreateSessionDir,
                "создан каталог сессий: /tmp/x".to_string(),
            ),
            FixResult::skipped(
                FixAction::TrimTranscripts,
                "квота не превышена: 1 КиБ из 100 МиБ".to_string(),
            ),
        ];
        let text = summary(&results);
        assert!(
            text.contains("Итог --fix: исправлено 1 из 2"),
            "text: {text}"
        );
        ...
        assert!(text
            .lines()
            .all(|l| !l.contains("⏭️") || l.contains("trim_transcripts")));

        assert_eq!(
            summary(&[]),
            "Итог --fix: действий не было — всё уже в порядке"
        );
    }
    ...
    fn fix_all_end_to_end_and_second_run_is_empty() {
        ...
        assert_eq!(
            results.len(),
            4,
            "TrimTranscripts не нужен — квота не превышена"
        );
        assert!(
            results.iter().all(|r| r.did_something),
            "первый прогон всё чинит"
        );
        let text = summary(&results);
